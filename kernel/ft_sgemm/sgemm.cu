...
void beam_line_trigger_handler() {
    time_t time = static_cast<time_t>(isr_timestamp/1000000000); // Convert nanoseconds to seconds
    printf("Trigger signal timestamp: %lu [ns], %s\n", isr_timestamp, ctime(&time));
    wait_trigger = 0;
}
...
void atexit_handler() {
    ...
    // Print end timestamp
    time(&timestamp);
    printf("End timestamp: %ld [seconds], %s", timestamp, ctime(&timestamp));
}

int main(int argc, char **argv){
    // Print start timestamp
    time(&timestamp);
    printf("Start timestamp: %ld [seconds], %s\n", timestamp, ctime(&timestamp));
    ...
    int kernel_number;
    int num_tests = 0;  // J - Not running any tests during profiling                          
    float alpha = 1.0;                      
    ...
    stat = gpioSetISRFunc(trigger_gpio, FALLING_EDGE, 1, &isr_timestamp, &beam_line_trigger_handler);
    if (stat < 0)
    {
    ...
    exit(0); // Exit normally to trigger atexit handler
}
